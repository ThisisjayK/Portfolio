...
class InfiniteGridMenu {
  ...
  // Added: track the animation frame and a disposed flag so the render loop
  // can be stopped on React unmount. The upstream React Bits component leaks
  // an ever-running rAF (and its WebGL context) on every unmount.
  #rafId = null;
  #disposed = false;
  ...
  run(time = 0) {
    // Added: bail out once disposed so the loop does not keep touching a GL
    // context whose canvas has been removed from the DOM.
  ...
  // Added: called from the React effect cleanup. Stops the loop and frees the
  // WebGL context so repeated tab switches don't exhaust the browser's cap.
  dispose() {
    this.#disposed = true;
    if (this.#rafId !== null) {
    ...
    // Only release the GL context on a real unmount (canvas detached from the
    // DOM). Under React StrictMode dev the cleanup fires on the throwaway mount
    // while the same canvas stays connected and is reused by the real mount;
    // losing the context there would leave the remount with a dead context.
    const gl = this.gl;
    if (gl && this.canvas && !this.canvas.isConnected) {
      gl.getExtension('WEBGL_lose_context')?.loseContext();
    }
  }

  #init(onInit) {
    // alpha:true lets the page background show through the canvas instead of
    // the opaque black the upstream component clears to.
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      // Added: a texture upload after dispose would throw on a lost context.
      if (this.#disposed) return;
  ...
  #render() {
    ...
    // Blend so the transparent parts of each logo tile composite over the page
    // background rather than showing the tile's own fill.
    gl.enable(gl.BLEND);
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }) {
  ...
  useEffect(() => {
    ...
    // Added: also re-sync the draw buffer when the canvas's own box changes
    // size (e.g. a CSS/layout change that doesn't fire a window resize), so the
    // sphere never renders into a stale viewport.
    ...
    return () => {
      ...
      // Added: stop the render loop and release the GL context on unmount.
      sketch?.dispose();
    };
  }, [items, scale]);
  ...
  const navigateTo = link => {
    ...
    if (link.startsWith('http')) {
      ...
      // Relative links navigate instead of only logging, so a link like
      // "kick/" opens the teardown page on the static site.
      window.location.href = link;
    }
  };

  // Prefer an in-app handler when the host provides one (e.g. opening the
  // teardown as an in-site page); otherwise fall back to link navigation.
  const activate = item => {
    if (!item) return;
    if (onItemClick) onItemClick(item);
    else navigateTo(item.link);
  };

  const handleButtonClick = () => activate(activeItem);

  // Added: treat a tap on the sphere (pointer up close to where it went down)
  // as a click on the front-most item, so clicking the logo opens its teardown.
  // A drag past the threshold is a rotation and is ignored here.
